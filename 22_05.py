    ...
    for i in range(1):
        ...
        #for sample_index in range(0, len(x_test) - window_size + 1):  # ウィンドウを1つずつずらして処理する
            ...
            with torch.no_grad():
                ...
                # x_test
                # 座標をモデルの入力に変換
                landmarks_tensor_x_test = torch.tensor(truth_landmarks, dtype=torch.float32).view(1, -1)
                landmarks_tensor_x_test = landmarks_tensor_x_test.to(device)

                # モデルに入力してじゃんけんの手を予測
                outputs_x_test = new_model(landmarks_tensor_x_test)
                _, predicted_x_test = torch.max(outputs_x_test.data, 1)
                gesture_label_x_test = predicted_x_test.item()

                # gesture_label を元にじゃんけんの手を表示
                janken_hand_x_test = janken_labels.get(gesture_label_x_test, 'Unknown')
                print(f'予測元: {janken_hand_x_test}')

            
                #  y_test
                # 座標をモデルの入力に変換
                landmarks_tensor_y = torch.tensor(ground_truth_landmarks, dtype=torch.float32).view(1, -1)
                landmarks_tensor_y = landmarks_tensor_y.to(device)

                # モデルに入力してじゃんけんの手を予測
                outputs_y = new_model(landmarks_tensor_y)
                _, predicted_y = torch.max(outputs_y.data, 1)
                gesture_label_y = predicted_y.item()

                # gesture_label を元にじゃんけんの手を表示
                janken_hand_y = janken_labels.get(gesture_label_y, 'Unknown')
                print(f'正解: {janken_hand_y}')


            
                # predicted_x
                # 座標をモデルの入力に変換
                landmarks_tensor_x = torch.tensor(sample_landmarks_x, dtype=torch.float32).view(1, -1)
                landmarks_tensor_x = landmarks_tensor_x.to(device)

                # モデルに入力してじゃんけんの手を予測
                outputs_x = new_model(landmarks_tensor_x)
                _, predicted_x = torch.max(outputs_x.data, 1)
                gesture_label_x = predicted_x.item()

                # gesture_label を元にじゃんけんの手を表示
                janken_hand_x = janken_labels.get(gesture_label_x, 'Unknown')
                print(f'予測: {janken_hand_x}')


                # プロットの設定
                save_path = f'/home/iwata/Pictures/hand_n{n_seq}_w{window_size}_{n_epochs}_{sample_index}.png'
                #save_path = f'/home/iwata/Pictures/hand_n{n_seq}_r{i+1}_w{window_size}_{n_epochs}_{sample_index}.png'
